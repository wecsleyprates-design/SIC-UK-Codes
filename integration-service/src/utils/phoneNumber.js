...
const regions = ["US", "AS", "GU", "MP", "PR", "VI", "CA", "GB", "MX", "IE"];
...
export const formatNumber = (number, format = PNF.E164) => {
	const result = phoneUtil.format(number, format);
	return result;
};
...
export const verifyAndFormatNationalNumber = number => {
	// Parse number with country code and keep raw input.
	const phoneNumber = parsePhoneNumber(number);
	if (!phoneUtil.isValidNumber(phoneNumber)) {
		throw httpError(StatusCodes.BAD_REQUEST, "Invalid phone number", ERROR_CODES.INVALID);
