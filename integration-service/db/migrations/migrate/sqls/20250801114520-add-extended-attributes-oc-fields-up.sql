...
CREATE OR REPLACE FUNCTION integration_data.extended_attributes (
        ...
        -- registry_match_mode text,
        registry_prediction numeric(5,4),
        
        -- ===== OpenCorporates Match Section =====
        registry_match_zip text,
        registry_match_city text,
        registry_match_state text,
        registry_match_name text,
        registry_match_address text,
        registry_match_company_number text,
        registry_match_jurisdiction_code text,
        registry_match_normalized_name text,
        registry_match_normalized_address text,
        
        -- ===== OpenCorporates Officers Array (Officer 1) =====
        registry_officer_1_name text,
        registry_officer_1_title text,
        registry_officer_1_address_street text,
        registry_officer_1_address_locality text,
        registry_officer_1_address_region text,
        registry_officer_1_address_postal_code text,
        registry_officer_1_address_country text,
        registry_officer_1_address_full text,
        registry_officer_1_first_name text,
        registry_officer_1_last_name text,
        registry_officer_1_status text,
        registry_officer_1_start_date text,
        registry_officer_1_person_uid text,
        registry_officer_1_person_number text,
        registry_officer_1_type text,
        registry_officer_1_source_url text,
        registry_officer_1_retrieved_at text,
        
        -- ===== OpenCorporates Officers Array (Officer 2) =====
        registry_officer_2_name text,
        registry_officer_2_title text,
        registry_officer_2_address_street text,
        registry_officer_2_address_locality text,
        registry_officer_2_address_region text,
        registry_officer_2_address_postal_code text,
        registry_officer_2_address_country text,
        registry_officer_2_address_full text,
        registry_officer_2_first_name text,
        registry_officer_2_last_name text,
        registry_officer_2_status text,
        registry_officer_2_start_date text,
        registry_officer_2_person_uid text,
        registry_officer_2_person_number text,
        registry_officer_2_type text,
        registry_officer_2_source_url text,
        registry_officer_2_retrieved_at text,
        
        -- ===== OpenCorporates Names Array =====
        registry_name_1_name text,
        registry_name_1_source text,
        registry_name_1_normalized text,
        registry_name_1_company_number text,
        registry_name_1_jurisdiction text,
        
        -- ===== OpenCorporates All Matches Array =====
        registry_match_1_prediction numeric(5,4),
        registry_match_1_name text,
        registry_match_1_company_number text,
        registry_match_1_jurisdiction text,
        registry_match_1_address text,
        
        -- ===== OpenCorporates Firmographic Fields =====
        registry_firm_name text,
        registry_firm_company_type text,
        registry_firm_current_status text,
        registry_firm_incorporation_date text,
        registry_firm_jurisdiction_code text,
        registry_firm_registry_url text,
        registry_firm_retrieved_at text,
        registry_firm_company_number text,
        registry_firm_previous_names text,
        registry_firm_dissolution_date text,
        registry_firm_inactive_date text,
        registry_firm_branch text,
        registry_firm_home_company text,
        registry_firm_registered_address text
    ) AS $$
...
opencorporates_data AS (
    SELECT business_id,
           requested_at as opencorporates_requested_at,
           response
    FROM integration_data.request_response r
    WHERE r.platform_id = 23 
      AND r.business_id = p_business_id
      AND r.request_type = 'fetch_business_entity_verification'
...
SELECT 
    COALESCE(e.business_id, z.business_id, oc.business_id),
    COALESCE(e.equifax_requested_at, z.zoominfo_requested_at, oc.opencorporates_requested_at),
    ...
    -- ===== OpenCorporates Top-Level Fields =====
    -- oc.response->>'match_mode',
    CASE WHEN oc.response->>'prediction' ~ '^[0-9]+\.?[0-9]*$' 
         THEN (oc.response->>'prediction')::numeric(5,4) 
         ELSE NULL END,
    
    -- ===== OpenCorporates Match Section =====
    oc.response->'match'->>'zip',
    oc.response->'match'->>'city',
    oc.response->'match'->>'state',
    oc.response->'match'->>'name',
    oc.response->'match'->>'address',
    oc.response->'match'->>'company_number',
    oc.response->'match'->>'jurisdiction_code',
    oc.response->'match'->>'normalized_name',
    oc.response->'match'->>'normalized_address',
    
    -- ===== OpenCorporates Officers Array (Officer 1) =====
    oc.response->'officers'->0->>'name',
    oc.response->'officers'->0->>'title',
    oc.response->'officers'->0->>'officer_address_street',
    oc.response->'officers'->0->>'officer_address_locality',
    oc.response->'officers'->0->>'officer_address_region',
    oc.response->'officers'->0->>'officer_address_postal_code',
    oc.response->'officers'->0->>'officer_address_country',
    oc.response->'officers'->0->>'officer_address_full',
    oc.response->'officers'->0->>'officer_first_name',
    oc.response->'officers'->0->>'officer_last_name',
    oc.response->'officers'->0->>'officer_status',
    oc.response->'officers'->0->>'officer_start_date',
    oc.response->'officers'->0->>'officer_person_uid',
    oc.response->'officers'->0->>'officer_person_number',
    oc.response->'officers'->0->>'officer_type',
    oc.response->'officers'->0->>'officer_source_url',
    oc.response->'officers'->0->>'officer_retrieved_at',
    
    -- ===== OpenCorporates Officers Array (Officer 2) =====
    oc.response->'officers'->1->>'name',
    oc.response->'officers'->1->>'title',
    oc.response->'officers'->1->>'officer_address_street',
    oc.response->'officers'->1->>'officer_address_locality',
    oc.response->'officers'->1->>'officer_address_region',
    oc.response->'officers'->1->>'officer_address_postal_code',
    oc.response->'officers'->1->>'officer_address_country',
    oc.response->'officers'->1->>'officer_address_full',
    oc.response->'officers'->1->>'officer_first_name',
    oc.response->'officers'->1->>'officer_last_name',
    oc.response->'officers'->1->>'officer_status',
    oc.response->'officers'->1->>'officer_start_date',
    oc.response->'officers'->1->>'officer_person_uid',
    oc.response->'officers'->1->>'officer_person_number',
    oc.response->'officers'->1->>'officer_type',
    oc.response->'officers'->1->>'officer_source_url',
    oc.response->'officers'->1->>'officer_retrieved_at',
    
    -- ===== OpenCorporates Names Array =====
    oc.response->'names'->0->>'name',
    oc.response->'names'->0->>'source',
    oc.response->'names'->0->>'normalized_name',
    oc.response->'names'->0->>'company_number',
    oc.response->'names'->0->>'jurisdiction_code',
    
    -- ===== OpenCorporates All Matches Array =====
    CASE WHEN oc.response->'all_matches'->0->>'prediction' ~ '^[0-9]+\.?[0-9]*$' 
         THEN (oc.response->'all_matches'->0->>'prediction')::numeric(5,4) 
         ELSE NULL END,
    oc.response->'all_matches'->0->'integration_business'->>'name',
    oc.response->'all_matches'->0->'integration_business'->>'company_number',
    oc.response->'all_matches'->0->'integration_business'->>'jurisdiction_code',
    oc.response->'all_matches'->0->'integration_business'->>'address',
    
    -- ===== OpenCorporates Firmographic Fields =====
    oc.response->'firmographic'->>'name',
    oc.response->'firmographic'->>'company_type',
    oc.response->'firmographic'->>'current_status',
    oc.response->'firmographic'->>'incorporation_date',
    oc.response->'firmographic'->>'jurisdiction_code',
    oc.response->'firmographic'->>'registry_url',
    oc.response->'firmographic'->>'retrieved_at',
    oc.response->'firmographic'->>'company_number',
    oc.response->'firmographic'->>'previous_names',
    oc.response->'firmographic'->>'dissolution_date',
    oc.response->'firmographic'->>'inactive_date',
    oc.response->'firmographic'->>'branch',
    oc.response->'firmographic'->>'home_company',
    oc.response->'firmographic'->>'registered_address'

FROM equifax_data e
FULL OUTER JOIN zoominfo_data z ON e.business_id = z.business_id
FULL OUTER JOIN opencorporates_data oc ON COALESCE(e.business_id, z.business_id) = oc.business_id;
