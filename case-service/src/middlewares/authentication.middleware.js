...
export const validateUser = async (req, res, next) => {
	try {
		...
		if (tokenData.iss.includes(envConfig.WORTH_ADMIN_USER_POOL_ID)) {
			tokenData.role = { id: ROLE_ID.ADMIN, code: ROLES.ADMIN };
		} else if (tokenData.iss.includes(envConfig.CUSTOMER_USER_POOL_ID)) {
			tokenData.role = { id: ROLE_ID.CUSTOMER, code: ROLES.CUSTOMER };
			tokenData.customer_id = tokenData["custom:customer_id"];
			tokenData.subrole_id = tokenData["custom:subrole_id"];
		} else {
			tokenData.role = { id: ROLE_ID.APPLICANT, code: ROLES.APPLICANT };
			tokenData.is_guest_owner = tokenData["custom:is_guest_owner"]
				? tokenData["custom:is_guest_owner"] === "true"
				: false;
			tokenData.issued_for = tokenData["custom:issued_for"] ? JSON.parse(tokenData["custom:issued_for"]) : null;
		}
