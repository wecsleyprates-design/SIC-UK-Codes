 ...
 * @return result
 */
export const sqlQuery = async <T extends QueryResultRow = any>({ sql, values }: { sql: string; values?: any[] }): Promise<QueryResult<T>> => {
...
export const beginTransaction = async () => {
	...
	try {
		...
		logger.debug(`sqlTransaction() begin`);
		return client;
	} catch (error: unknown) {
		throw new DatabaseError((error as any).message);
	}
...
export const rollbackTransaction = async ({ client }) => {
	if (typeof client !== "undefined" && client) {
		try {
			...
			logger.debug(`sqlTransaction() rollback`);
		} catch (error) {
			throw new DatabaseError((error as any).message);
		} finally {
			client.release();
...
export const commitTransaction = async ({ client }) => {
	if (typeof client !== "undefined" && client) {
		try {
			...
			logger.debug(`sqlTransaction() commit`);
		} catch (error) {
			throw new DatabaseError((error as any).message);
		} finally {
			client.release();
...
export const sqlTransaction = async (queries, queryValues) => {
	...
	try {
		const queryPromises: Promise<QueryArrayResult<any>>[] = [];
		queries.forEach((query, index) => {
			logger.debug(`sqlTransaction() query: ${query} | data: ${queryValues[index]}`);
...
export const sqlExecSingleRow = async ({ client, sql, values }) => {
	...
	try {
		...
		logger.error({ error, sql, values }, "sqlExecSingleRow() error");
