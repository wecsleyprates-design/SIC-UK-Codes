...
import { init, type LDOptions, type LDClient, type LDContext, type LDFlagValue } from "@launchdarkly/node-server-sdk";
import { logger } from "./logger";
import { FileDataSourceFactory } from "@launchdarkly/node-server-sdk/integrations";
import { getLaunchDarklyFilePath } from "#utils/index";
...
export async function initializeLDClient() {
	...
	launchDarklyClient = init(envConfig.LD_SDK_KEY, options);
...
export async function getFlagValue(key: string, user?: LDContext | null, defaultValue: any = false): Promise<LDFlagValue> {
...
export async function featureFlagStatus(featureFlagKey: string, userID: string = "anonymous", additionalAttributes = {}) {
	...
	try {
		...
		logger.error({ error: err, featureFlagKey, user, additionalAttributes }, "failed to retrieve feature flag status");
	}
